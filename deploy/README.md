...
Checkout supports **LiqPay** (`paymentMethod=liqpay`). Offline methods (bank transfer / pickup) stay available.

1. Create a merchant in [LiqPay](https://www.liqpay.ua/) and copy **public** / **private** keys into `.env`:
   - `LIQPAY_PUBLIC_KEY=...`
   - `LIQPAY_PRIVATE_KEY=...`
2. Production requires a **public HTTPS** site:
   - Domain DNS → static IP
   - Caddy site block with the domain (see comments in [`Caddyfile`](Caddyfile))
   - Publish **443** in Compose and open it in the Lightsail firewall (callbacks come from LiqPay, so IP whitelist on 443 will block payments)
   - `CLIENT_URL` / `API_URL` = `https://your-domain`
3. Callback URL used by the API: `{API_URL}/api/payments/liqpay/callback`
4. After payment the buyer returns to `{CLIENT_URL}/checkout/result?orderId=...`

Without keys, selecting online payment returns HTTP 503 from `POST /api/orders`.
